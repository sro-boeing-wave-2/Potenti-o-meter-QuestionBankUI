...
// import { DialogDataExampleDialog, QuestionType } from '../app.component';
import { QuestionService } from '../../service/question.service';
import { MMCQRecord, MMCQOption } from 'src/app/Models/MMCQRecord';
import { MCQRecord, MCQOption } from '../Models/MCQRecord';
// import { QuestionType } from '../app.component';
...
@Component({
  selector: 'app-import-questionbank',
  templateUrl: './import-questionbank.component.html',
  styleUrls: ['./import-questionbank.component.css']
})
export class ImportQuestionbankComponent implements OnInit {
  ...
  openDialog() {
    ImportQuestionbankComponent.dialogRef = this.dialog.open(DialogDataExampleDialog, {
    });
    const mcq = ImportQuestionbankComponent.dialogRef.componentInstance.mcqQuestion.subscribe((result) => {
      this.inputComponent.Questions.push(result);
      this.inputComponent.dataSource.paginator = this.inputComponent.paginator;
    });
    const mmcq = ImportQuestionbankComponent.dialogRef.componentInstance.mmcqQuestion.subscribe((result) => {
      this.inputComponent.Questions.push(result);
      this.inputComponent.dataSource.paginator = this.inputComponent.paginator;
    });
  }
      ...
      this._questionService.postQuestions(element).subscribe(result => {
        if(result.statusText == "OK")
        {
          ...
          if(element.QuestionType=="MCQ")
          {
            ...
            ImportQuestionbankComponent.dialogRef.close();
          }
          else {
